...
vorwarnzeit_berechnen_AG <- function(ngesamt,cases,faelle,Kapazitaet_Betten,Rt=1.3, hospquote){
  ...
  for (i in 1:3) {
    mysir <- sirmodel(ngesamt = ngesamt[i],
                      ...
                      R0 = Rt,
                      gamma = gamma,
                      horizont = 365) %>% mutate(Neue_Faelle_hq=hospquote[i]*(I-lag(I)+R-lag(R)))
    mysir_AG[[i]] <- mysir
  }
