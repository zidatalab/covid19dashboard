...
SIHRS <- function(time, state, parameters, ngesamt, gamma, qa, delta, nu, omicron) {
  par <- as.list(c(state, parameters))
  with(par, {
    dS <- -beta/ngesamt * (I+H) * S + omicron * R
    dI <- beta/ngesamt * (I+H) * S - (1-qa) * gamma * I - qa * delta * I
    dH <- qa * delta * I - nu * H
    dR <- (1-qa) * gamma * I + nu * H - omicron * R
    list(c(dS, dI, dH, dR))
  })
}
...
sihrsmodel<- function(ngesamt, S, I, H, R, 
                      R0, gamma, qa, delta, nu, omicron, 
                      horizont=365) {
  # Set parameters
  ## Infection parameter beta; gamma: recovery parameter
  params <- c("beta" = R0*((1-qa)*gamma+qa*delta))
  ...
  # Solve using ode (General Solver for Ordinary Differential Equations)
  out <- ode(y = init, times = times, func = SIHRS, 
             parms = params, 
             ngesamt=ngesamt, gamma=gamma, qa=qa, 
             delta=delta, nu=nu, omicron=omicron)
