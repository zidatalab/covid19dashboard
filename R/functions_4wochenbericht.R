...
rki <- rki %>% mutate(Meldedatum=as_date(Meldedatum)) %>% filter(Meldedatum<=stichtag)
brd_timeseries <- brd_timeseries %>% filter(date<=stichtag)
...
sum((ausgangsdaten%>%filter(id>16 &date==stichtag))$`Faelle_letzte_7_Tage_je100TsdEinw_60+`>=35, na.rm=TRUE)
