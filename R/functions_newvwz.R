...
blid_id <- rki %>% select(IdBundesland, IdLandkreis) %>% 
  mutate(IdLandkreis=as.integer(IdLandkreis)*1000) %>% 
  distinct() %>% 
  drop_na() %>% 
  bind_rows(tibble(IdBundesland=0:16, IdLandkreis=0:16))
...
ausgangsdaten <- letzte_7_tage %>%
  ...
  mutate(Faelle_letzte_7_Tage_je100TsdEinw=round(Faelle_letzte_7_Tage/((EW059+EW6079+EW80)/100000)),
         Faelle_letzte_7_Tage_je100TsdEinw=ifelse(Faelle_letzte_7_Tage_je100TsdEinw<0,0,Faelle_letzte_7_Tage_je100TsdEinw),
         Kapazitaet_Betten=(faelle_covid_aktuell+betten_frei)) %>% 
  mutate(EW60p=EW6079+EW80,
...
ausgangsdaten_ror <- ausgangsdaten %>%
  ...
  summarise(EW059=sum(EW059, na.rm = TRUE),
            ...
            .groups="drop") %>% 
  mutate(IdBundesland=round(ROR11/100)) %>% 
  mutate(EW60p=EW6079+EW80,
         Infected60p=Infected6079+Infected80,
         cases60p=cases6079+cases80) %>% 
  left_join(impfquoten_voll, by=c("IdBundesland"="id", "date")) %>% 
...
myTage <- ausgangsdaten %>% filter((date>=as_date("2020-03-13") & id<=16) |
  ...
  do(Tage = vorwarnzeit_berechnen_AG2(ngesamt = c(.$EW059, .$EW60p),
                                     ...
                                     Rt = 1.3,
                                     icurate_altersgruppen = c(.$icurate_mitgeimpften_60minus, 
...
vacc_bl <- rki_vacc %>%
  filter(date==max(rki_vacc$date) &
           metric %in% c("personen_min1_kumulativ")) %>%
  group_by(geo) %>% 
  pivot_wider(names_from = metric, values_from = value) %>% 
  mutate(impfungen_initial=personen_min1_kumulativ)
...
vaccination_sti_its_death_data <- rki_vacc %>%
  ...
  left_join(., jhu_germany %>% select(date, incident_mean7_deaths), 
            by=c("datum"="date"))
...
vacc_gesamt <- rki_vacc_complete %>%
  filter(metric=="personen_min1_kumulativ" | 
...
#   filter(metric=="impf_quote_min1_alter_60plus" |
#            metric=="impf_quote_voll_alter_60plus") %>%
#   filter(date==max(date)) %>%
#   left_join(pflegeheimbewohnende_2019_bundeslaender %>% 
...
#   filter(metric=="impf_quote_min1_alter_unter60" | 
#            metric=="impf_quote_voll_alter_unter60") %>%
#   filter(date==max(date)) %>%
#   left_join(pflegeheimbewohnende_2019_bundeslaender %>% 
#               select(Bundesland, Bundesland_ID),
#             by=c("geo"="Bundesland")) %>%
...
vacc_alle_faktenblatt <- vacc_alle %>%
  ...
  mutate(quote_gesamt=value_personen_min1_kumulativ/population_personen_min1_kumulativ,
         quote_unvollst_gesamt=(value_personen_min1_kumulativ-value_personen_voll_kumulativ)/population_personen_min1_kumulativ,
         quote_vollst_gesamt=(value_personen_voll_kumulativ)/population_personen_voll_kumulativ,
         quote_auffr_gesamt=(value_personen_auffr_kumulativ)/population_personen_auffr_kumulativ,
...
vacc_table <- vacc_alle_faktenblatt %>%
  mutate(Bundesland=ifelse(geo=="Deutschland", "Gesamt", geo),
         ...
         "Impfungen pro 100k EW"=format(round(impfungen_gesamt/population_personen_min1_kumulativ*100000), decimal.mark=",")
  ) %>%
...
vacc_table_vaccsim <- bind_rows(
  rki_vacc %>%
    filter(date==max(date) & (metric=="dosen_kumulativ" |
                                metric=="personen_voll_kumulativ")),
  rki_vacc %>%
    filter(date==rki_vacc_vortag & (metric=="dosen_kumulativ" |
                                      metric=="personen_voll_kumulativ")),
  rki_vacc %>%
    filter(date==rki_vacc_vor7tag & (metric=="dosen_kumulativ" | 
                                       metric=="personen_voll_kumulativ")) # -6?
) %>%
...
vacc_table_vaccsim <- vacc_table_faktenblatt %>%
  ...
  rename(`Gesamt 2x`=`Gesamt vollst.`)
