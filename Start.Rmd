...
```{r indicatorVacc}
indicatorVacc <- vaccinations %>% filter(datum==max(vaccinations$datum) &
                                           region=="DE" &
                                           metric=="impfungen_pro_1000_einwohner") %>% pull(value)
indicatorVacc <- paste0(format(round(indicatorVacc/10, 1), decimal.mark = ","), "%")
