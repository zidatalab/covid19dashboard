...
```{r indicatorVWZ}
vorwarnzeitaktuell <- vorwarnzeitergebnis %>% filter(id==0) %>% pull(Vorwarnzeit)
valueBox(paste(vorwarnzeitaktuell,"Tage"),
         caption=paste0("Vorwarnzeit bis Erreichen der Kapazitätsgrenze \n effektive Vorwarnzeit: ", vorwarnzeitergebnis %>% filter(id==0) %>% pull(Vorwarnzeit_effektiv), " Tage"),
...
```{r plotagefatality}
ggplotly(age_plot_fatality, tooltip = c("x", "y")) %>%
...
```{r plotBLR}
ggplotly(plot_rwert_bund, tooltip = c("x", "y", "text")) %>%
  config(displayModeBar = FALSE, responsive=TRUE)
...
```{r plotVWZR}
ggplotly(vorwarnzeitverlauf_plot, tooltip = c("x", "y", "text")) %>%
  config(displayModeBar = FALSE, responsive=TRUE)
```
